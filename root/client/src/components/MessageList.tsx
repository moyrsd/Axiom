              ...
              <ReactMarkdown
                ...
                components={{
                  ...
                  blockquote: ({ node, ...props }) => (
                    <blockquote className="border-l-4 border-gray-500 pl-4 py-1 my-3 italic" {...props} />
                  ),
                  
                  // Fixed code handling to avoid <pre> inside <p>
                  code: ({ node, inline, className, children, ...props }) => {
                    const match = /language-(\w+)/.exec(className || '');
                    
                    if (inline) {
                      return (
                        <code className="bg-gray-700 px-1 rounded text-xs" {...props}>
                          {children}
                        </code>
                      );
                    }
                  ...
                  th: ({ node, ...props }) => (
                    <th className="px-4 py-2 text-left text-xs font-medium uppercase" {...props} />
                  ),
                  td: ({ node, ...props }) => (
